...
import com.example.boki.ExpensesFragment;
...
public class ExpenseRepository {
    ...
    /**
     * NEW: Retrieves the sum of expenses for each category within a specific date range.
     * This uses a GROUP BY SQL query to aggregate the data.
     *
     * @param startDate The start date of the period (inclusive), formatted as "YYYY-MM-DD".
     * @param endDate   The end date of the period (inclusive), formatted as "YYYY-MM-DD".
     * @return A list of CategoryExpense objects for the specified period.
     */
    public List<ExpensesFragment.CategoryExpense> getCategoryExpenseSummary(String startDate, String endDate) {
        // The return type now points to the nested class inside ExpensesFragment
        List<ExpensesFragment.CategoryExpense> summaryList = new ArrayList<>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = null;

        // The SQL query to group by category and sum amounts within a date range
        String query = "SELECT " + ExpenseDbHelper.COLUMN_CATEGORY + ", " +
                "SUM(" + ExpenseDbHelper.COLUMN_AMOUNT + ") as total " +
                "FROM " + ExpenseDbHelper.TABLE_EXPENSE + " " +
                "WHERE " + ExpenseDbHelper.COLUMN_DATE + " BETWEEN ? AND ? " + // The WHERE clause
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                if (categoryIndex != -1 && totalIndex != -1) {
                    String category = cursor.getString(categoryIndex);
                    double totalAmount = cursor.getDouble(totalIndex);

                    // THIS IS THE CORRECTED AND COMPLETED LINE
                    summaryList.add(new ExpensesFragment.CategoryExpense(category, totalAmount));
                }
            }
        } finally {
            if (cursor != null) {
